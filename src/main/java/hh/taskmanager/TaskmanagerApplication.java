...
@Bean
	public CommandLineRunner seedData(TaskRepository taskRepository, CategoryRepository categoryRepository, ProjectRepository projectRepository, AppUserRepository appUserRepository, PasswordEncoder passwordEncoder) {
		return (args) -> {
			...
			taskRepository.save(new Task("Go to the shop", null, LocalDateTime.parse("2026-02-01T12:00"), false, false, errands, null, user));
			taskRepository.save(new Task("Meet up with Lisa", null, LocalDateTime.parse("2026-03-11T15:10"), false, false, personal, null, user));
			taskRepository.save(new Task("Doctor's appointment", "Ask for a new prescription", null, true, false, other, null, user));

			Log.info("------ Mock-data: tasks without projects created ------");

			Project taskManagerApp = projectRepository.save(new Project("TaskManager", "My Spring Boot application", false, user));
			Project diyPorch = projectRepository.save(new Project("DIY Porch", "Build a porch for the backyard",true, user));
			...
			taskRepository.save(new Task("Make test data for TaskManager", "The data will be used to test the application", LocalDateTime.parse("2026-01-15T08:00"), false, false, work, taskManagerApp, user));
